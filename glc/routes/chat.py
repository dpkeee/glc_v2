...
async def embed(req: EmbedRequest, request: Request):
    ...
    try:
        ...
        if req.provider:
            ...
            if e.status == 400:
                raise HTTPException(400, "upstream provider rejected embed request")
            raise HTTPException(502, "upstream provider embed request failed")
        raise HTTPException(503, "no embedding providers available")
